...
double StringH::Parse_Double(char* double_string)
{
    ...
    for(int i=start_index; i<strlen(double_string); ++i )
    {
     ...
     double multiplier = 0;
    if(double_string[end_index+1] == 'e')
    {
...
bool StringH::Strings_Match(char* string1, char* string2)
{
    int index = 0;

    StringH::Trim_WhiteSpace(string1);
    StringH::Trim_WhiteSpace(string2);

    while( index<strlen(string1) && index<strlen(string2) )
    {
...
void Convert_Character(char* line, char orig_char, char change_char)
{
    if(line == NULL)
        return;
...
bool StringH::Equals(char* string1, const char* string2)
{
    int index = 0;
