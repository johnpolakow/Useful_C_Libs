...
USB_Device** USB::Search_ttyUSB_Devices()
{
    ...
    USB_Device** device_list = (USB_Device**)malloc( sizeof(USB_Device*)*5);    // Assume max 5 devices connected
    for( int i=0; i<5; ++i)
        device_list[i] = NULL;
...
void USB::Get_USB_Device_ID(USB_Device* device)
{
    ...
    USB::ttyUSB_Send_Command(*device, "*RST");  // reset the device now while searching attached USB devices, so we don't have to wait for the reset later during setup
}
