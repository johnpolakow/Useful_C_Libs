...
Timestamp::Timestamp()
{
    ...
    timeinfo = localtime(&rawtime);
    strftime (target_string, 10,"%H",timeinfo);     // write "hour" value to temp string
    this->time.hour = StringH::Parse_Int(target_string);    // parse the hour and assign to timestamp

    StringH::Erase_String( target_string);
    strftime (target_string, 10,"%M",timeinfo);     // write "minute" val to temp string
    this->time.minute = StringH::Parse_Int(target_string);  // parse the minute val, assign to class variable
